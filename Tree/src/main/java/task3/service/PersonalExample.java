...
public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < openTags.length; i++) {
        ...
        //if (isSingleTag) {
            ...
            Matcher xmlMatcher = xmlPatternStart.matcher(xmlInfo.toString());

            while (xmlMatcher.find()) {

                int start = xmlMatcher.start();
                int end = xmlMatcher.end();
                String openTag = xmlInfo.toString().substring(start, end);
                System.out.print(openTag);

                if (openTags[i].substring(1).contains(closeTags[j].substring(2, closeTags[j].length() - 2)))
                     System.out.println(s=xmlInfo.toString().substring(xmlInfo.toString().indexOf(openTags[i])+openTags[i].length()+1,xmlInfo.toString().indexOf(closeTags[j]))+"FGBFG");
            ...
            Node Xmlnode = new Node(openTags[i],s);
